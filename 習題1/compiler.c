...
void next_token() {
    ...
    // 辨識識別碼 (ID) 與 關鍵字 (Keyword)
        ...
        else if (strcmp(cur_token.text, "while") == 0) cur_token.type = TK_WHILE; // 新增 while 關鍵字
...
void statement() {
    ...
    // 新增 while 迴圈解析邏輯
        ...
        emit("JMP", "-", "-", back_addr); // 無條件跳回條件判斷起點
...
void vm() {
    ...
    while (pc < quad_count) {
        ...
        else if (strcmp(q.op, "JMP") == 0) { // 新增無條件跳轉處理
            pc = atoi(q.result) - 1;
        }
...
int main(int argc, char *argv[]) {
    // 簡單的除錯/測試處理：若未提供檔案，直接執行內建字串來測試 while
    if (argc < 2) {
        printf("未提供檔案，將編譯並執行內建測試碼...\n\n");
        src = "i = 0; while (i < 5) { i = i + 1; }";
    } else {
